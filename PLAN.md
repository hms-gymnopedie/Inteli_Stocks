...
| ID | Task | 파일 | Agent | Status | Notes |
|---|---|---|---|---|---|
| B2-MAP | WorldMap 재구현 (TopoJSON + d3-geo, 줌·팬, 레이어, 핀 클릭) | `app/src/lib/WorldMap/*` | frontend-ui-integrator | ✅ | TopoJSON(`world-atlas/countries-110m.json`) + `geoEqualEarth().fitSize([1000,500])`. heat 키 ISO 3166-1 alpha-3, M49→ISO-3 lookup 내장. pins lat/lng 우선, 레거시 x/y 폴백. 4-commit split (`0f09a7f` → `c87cf5a` → `746a2e8` → `5b7a7fe`). **B2-MAP-2 (`952ee0d`)**: ✅ 줌·팬·핀 클릭. `onPinClick` first-class API (pin → focusable `<g role="button">` w/ Enter·Space + `aria-label`). `interactive` prop (default true) + `lib/WorldMap/zoom.ts` 헬퍼: wheel zoom (1×–8×, cursor-anchored), pointer drag pan, 2-finger pinch + 1-finger touch pan, 키보드(Arrow*/+/-/0), 우상단 ⌖ 리셋 버튼. `pages/geo/WorldMap.tsx` 오버레이 버튼 핵 제거 → 5-line pass-through. WORLDMAP-2 styles section staging-bleed via `0536bbc`. |
...
| B4-RS | 반응형/모바일 (현재 1100px 이하 sidebar 숨김 → 본격 대응) | 각 페이지 + `styles.css` | frontend-ui-integrator | ✅ | Breakpoints 1100/900/720/600/430. Topbar `≡` hamburger at <=720px. KPI 4→2→1, Valuation 6→3→2, Equity+Allocation stacks. Holdings horizontally scrolls inside its panel (min-width 720). Geo overlay un-positions at <=600px; Tweaks FAB shrinks. CSS fenced under `/* === RESPONSIVE (B4-RS) === */`. (commits `2c76110` topbar+CSS · `b441536` Overview · `157ae42` Portfolio · `e315029` Geo+Detail) |
...
- ✅ **B29** — Trades ↔ Portfolio 연동 + Trades 행 수정 (트리거 포함).
  - **Auto-portfolio**: `POST /api/portfolio/trades` 의 BUY 트레이드는 `ensureHoldingForTrade()` 헬퍼로 holdings 에 종목이 없으면 yahoo `quote()` 로 name/currency 받아 자동 생성. SELL 은 holdings 안 건드림. 이미 추적 중이면 idempotent.
  - **Trade edit**: `PUT /api/portfolio/trades/:idx` 추가 — 트레이드 전체 교체 + 같은 ensureHolding 사이드이펙트.
  - **Rationale edit**: `PUT /api/positions/:id` + storage `updateRationale(id, patch)` — reason / entryPrice / triggers 패치, `firedAt`/`notified` 리셋해서 재발화 가능.
  - **TradesLog UI**: 단순 폼을 풀 폼으로 교체 (`TradeFormFull.tsx` 신규). Edit (✎) 버튼 행마다 추가, 폼은 add/edit 통합 — BUY 트레이드는 reason + 5종 sell 트리거 (sell-by-date · 절대가격 위/아래 · 진입가 대비 % · trailing peak %) 인라인 편집. 각 BUY 행에 활성 트리거 개수 별표 (★N) 배지. 저장 시 트레이드 + (reason+triggers 있으면) `addPosition`/`updatePosition`, 둘 다 비면 기존 rationale `deletePosition`. positions 는 symbol 로 1:1 매칭.
  - 라이브 검증: 새 PG (Procter & Gamble) BUY → holdings 19→20, name="Procter & Gamble Company (The)" 자동. SELL은 holdings 변동 없음. PUT rationale: triggers 1→2, firedAt 리셋 확인.
- ✅ **B28** — Leaderboard 전략 수정. `PUT /api/sim/strategies/:id` (백테스트 재실행 후 같은 id+createdAt로 덮어쓰기) + storage `updateStrategy`. Frontend: `formMode` 유니온 (closed | create | edit), 행마다 `✎` 버튼 → NewStrategyForm prefilled (initial prop) + "Save changes" 버튼. Smoke test: AAPL/MSFT/NVDA → SPY/QQQ/GLD swap, +35% → +79% 갱신, id/createdAt 보존.
