...
function describeTrigger(t: { type: string; price?: number; date?: string; pct?: number; basePrice?: number; peakPrice?: number }): string {
  switch (t.type) {
    case 'date':              return `by ${t.date}`;
    case 'absoluteAbove':     return `≥ $${(t.price ?? 0).toFixed(2)}`;
    case 'absoluteBelow':     return `≤ $${(t.price ?? 0).toFixed(2)}`;
    case 'pctFromBase':       return `${(t.pct ?? 0) >= 0 ? '+' : ''}${t.pct}% from $${(t.basePrice ?? 0).toFixed(2)}`;
    case 'trailingFromPeak':  return `trailing ${t.pct}% from $${(t.peakPrice ?? 0).toFixed(2)}`;
    default:                  return t.type;
  }
}

function PositionsSection() {
  const [items,   setItems]   = useState<PositionItem[] | null>(null);
  ...
  async function refresh(): Promise<void> {
    try {
      ...
      const j = (await r.json()) as PositionItem[];
      setItems(j);
    } catch (e) {
  ...
  async function remove(id: string, label: string): Promise<void> {
    if (!window.confirm(`Cancel position rationale for ${label}?`)) return;
    setBusy(true); setMessage(null);
    try {
      await fetch(`/api/positions/${encodeURIComponent(id)}`, { method: 'DELETE' });
      await refresh();
    } catch (e) {
  ...
  return (
    <section className="settings-section settings-section--wide">
      <h2 className="settings-section-h">
        Position Rationales <span className="muted-2" style={{ fontSize: 11 }}>· buy reasons + sell triggers</span>
      </h2>
      <p className="settings-section-desc">
        Created when you log a BUY trade on a Detail page. Sell triggers are
        evaluated hourly during US market hours; matched triggers post a Slack
        message (configure <code>SLACK_WEBHOOK_URL</code> above).
      </p>
      <div className="settings-actions">
        <button
          type="button"
      ...
      {items === null ? (
        ...
        <div className="settings-foot muted">
          No rationales yet. Add a BUY trade on any Detail page (the chip lets
          you enter a reason + sell triggers).
        </div>
      ) : (
        <>
          {active.length > 0 && (
            <div className="positions-list">
              <div className="wf-mini muted-2" style={{ marginTop: 6 }}>
                ACTIVE · {active.length}
              </div>
              {active.map((p) => (
                <div key={p.id} className="positions-row">
                  <div className="row between center">
                    <div className="row gap-2 center">
                      <span className="ticker">{p.symbol}</span>
                      <span className="wf-mini muted">@ ${p.entryPrice.toFixed(2)}</span>
                  ...
                  <div className="positions-reason">{p.reason}</div>
          ...
          {fired.length > 0 && (
            <div className="positions-list">
              ...
              {fired.map((p) => (
                <div key={p.id} className="positions-row positions-fired">
                  <div className="row between center">
                    <div className="row gap-2 center">
                      <span className="ticker">{p.symbol}</span>
                      <span className="wf-mini muted">@ ${p.entryPrice.toFixed(2)}</span>
                      <span className="wf-mini" style={{ color: 'var(--orange)' }}>
                        ✓ {p.firedTrigger ? describeTrigger(p.firedTrigger) : 'fired'}
                      </span>
                      <span className="wf-mini muted-2">
                        {p.firedAt ? new Date(p.firedAt).toLocaleString() : ''}
                      </span>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className="positions-reason muted">{p.reason}</div>
                </div>
              ))}
            </div>
          )}
